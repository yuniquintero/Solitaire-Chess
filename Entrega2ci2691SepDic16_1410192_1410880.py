...
def init():
	...
	global movement, chess, window, FINISHED, count, states, states, textMode, jugar, partida, sec, lineaPartida, wins, temp, recordLv, pag, lastPage, option
	emptyBoard()
	temp = 0
	movement = ""
	chess = ""
...
def getMovement():
	...
	assert cont >= 1 and len(states) >= 0 and len(grid) > 0 and len(movement) > 0, str(cont) + " " + str(len(states)) + " " + str(len(grid)) + " " + str(len(movement))
